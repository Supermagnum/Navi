...
**Providers:** free/open APIs preferred (e.g. Open-Meteo, national met
services). Weather Underground-class feeds only where Terms of Use and API keys
allow; keys stay in host secrets, never in WASM.
...
|---|---|
| HTTPS fetch (opt-in network), rate-limit, SQLite cache | **Host** |
...
| `weather_read` (new) | Read cached samples near lat/lon / along corridor |
| `log` | Diagnostics |
...
2. Implement host fetch + SQLite cache + `weather_read` capability.
3. Map Open-Meteo (or chosen provider) codes → Meteocons slugs via manifest.
