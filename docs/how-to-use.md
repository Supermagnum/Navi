...
| **Download region + build place index** | Probes the optional pack host (`current.json`), then downloads the Geofabrik PBF (pack-fetch not implemented yet — always this path today), binds the region, builds the place search index, and builds indexed routing maps when possible. Soft fallback if the host is down or the region is unpublished — no hard error. To point at another pack host, see [`pack-server-client.md`](pack-server-client.md#changing-the-pack-server-address). |
