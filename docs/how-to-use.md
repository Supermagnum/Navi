...
| **Download region** / **Download region + build place index** | When the pack server lists the selected path (green pill), the button is green and labeled **Download region** — installs published packs, then downloads the Geofabrik extract, builds the place index, and range-extracts the Protomaps basemap for the same path. Otherwise it stays the default color and runs Geofabrik PBF download + place index + local pack bake + basemap. |
...
| **Check for OSM updates** | Opt-in: pack-server `current.json` generation first, Geofabrik `state.txt` if the host is unreachable (never silent). Green when the selected path is pack-ready (same readiness as region pills). |
