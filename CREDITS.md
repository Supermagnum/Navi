...
`alpine_hut`, `townhall`, `car_repair`, `motorcycle`, `motorcycle_repair`, `bicycle`,
...
- **Source tree:** `symbols/` in the Carto repository (e.g. `shop/alcohol.svg`,
  ...
  `religion/christian.svg`, `natural/spring.svg`, `tourism/alpinehut.svg`)
...
| `alpine_hut` | `tourism/alpinehut.svg` (Carto filename has no underscore) |
| `wilderness_hut` | shares `alpine_hut` sprite (same unstaffed mountain-hut semantics) |
...
## Shelter sprite (offline basemap)

Protomaps `pois.kind=shelter` uses the Navit `shelter.svg` icon packed into the
same atlas as sprite key `shelter` (via `scripts/build-poi-sprites.sh`).

- **Source:** [`core/src/icons/shelter.svg`](core/src/icons/shelter.svg)
- **License:** GPL v2 (Navit-derived; same family as other Navi overlay icons —
  see [`docs/icons.md`](docs/icons.md))

`chalet` keeps the Carto `hotel` sprite as closest staffed/paid-lodging match;
dedicated chalet art is not in the atlas.
